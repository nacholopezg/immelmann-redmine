...
class AgileSprintsQuery < AgileQuery
  ...
  def initialize_available_filters
    ...
    add_available_filter('sprint_id', type: :list, values: project.agile_sprints.sorted.map { |v| [v.name, v.id.to_s] })
  ...
  def sprints
    ...
    project.agile_sprints.where(sprints_sql.join(' AND ')).sorted
  end

  def no_sprint_issues(params = {})
    return @no_sprint_scope if @no_sprint_scope
    q = (params[:q] || params[:term]).to_s.strip
    @no_sprint_scope = Issue.visible.joins(query_includes)
    if project
    ...
    if q.present?
      if q.match(/^#?(\d+)\z/)
        @no_sprint_scope = @no_sprint_scope.where("(#{Issue.table_name}.id = ?) OR (LOWER(#{Issue.table_name}.subject) LIKE LOWER(?))", $1.to_i, "%#{q}%")
      else
  ...
  def sprint_issues(sprint)
    ...
    @sprint_issues_cashe[sprint.id] = sprint.issues.visible.joins(query_includes)
                                                   .where(statement)
                                                   .sorted_by_rank
  end
