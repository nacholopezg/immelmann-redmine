...
class AgileQuery < Query
  ...
  def sprint
    @sprint ||= project.agile_sprints.where(id: sprint_id).first
  end

  def sprint_id
    return nil unless project
    @sprint_id ||= options[:sprint_id] || project.active_sprint.try(:id) || project.agile_sprints.first.try(:id)
  end
  ...
  def issue_scope
    ...
    @agile_scope = @agile_scope.where("#{AgileData.table_name}.agile_sprint_id = ?", sprint_id) if sprints_enabled.to_i > 0
    @agile_scope
  end
