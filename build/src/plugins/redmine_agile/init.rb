...
Redmine::Plugin.register :redmine_agile do
  ...
  menu :project_menu, :backlog, { controller: 'agile_versions', action: 'sprints' }, caption: :label_agile_board_backlog,
                                                                                     after: :gantt,
                                                                                     param: :project_id,
       if: Proc.new { User.current.allowed_to?(:manage_agile_verions, nil, global: true) }
  ...
  project_module :agile do
    ...
    permission :manage_backlog, { agile_boards: [:backlog_load_more, :backlog_autocomplete] }
  end
end
