    ...
    module Helpers
      module ChartHelper
        ...
        def tooltip_resource_booking_attributes(resource_booking)
          ...
          dates_text = dates_range_label(resource_booking.start_date, resource_booking.get_end_date, '%a, %d %b')
          scheduled_text = scheduled_label(hours_per_day, working_days_amount, non_working_days_amount)
          total_scheduled_text = total_scheduled_label(hours_per_day, working_days_amount, non_working_days_amount)

          [{ name: @cached_label_dates,     value: dates_text },
           { name: @cached_label_scheduled, value: scheduled_text },
           { name: @cached_label_total,     value: total_scheduled_text }]
        end
        ...
        def subject_for_user(user)
          content_tag :div, id: "user-#{user.id}", class: 'user-subject' do
            [content_tag(:span, '&nbsp;'.html_safe, class: 'expander', onclick: %($('.user-resource-bookings[group_id="#{user.id}"]').toggleClass('open');)),
             avatar(user, size: '16'),
             link_to_user(user)].join(' ').html_safe
          end
        end
        ...
        def zoom_link(rb_chart, in_or_out)
          case in_or_out
          when :in
            if rb_chart.zoom < ResourceBookingChart::MAX_ZOOM
          ...
          when :out
            if rb_chart.zoom > ResourceBookingChart::MIN_ZOOM
        ...
        def hours_per_day_bar_label(resource_booking)
          "#{to_int_if_whole(resource_booking.hours_per_day)} #{l(:field_hours_per_day_abbreviation)}"
        end
