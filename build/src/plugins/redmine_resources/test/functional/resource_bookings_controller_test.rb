...
class ResourceBookingsControllerTest < ActionController::TestCase
  ...
  def test_should_not_update_resource_booking_when_end_date_more_than_issue_due_date
    @request.session[:user_id] = @admin.id
    rb_params = @resource_booking_params.merge(start_date: Date.today, end_date: 10.day.from_now.to_date)
    ...
    rb_params = rb_params.merge(end_date: 11.day.from_now.to_date)
    should_not_update_resource_booking :success, id: 1, resource_booking: rb_params
    should_not_update_resource_booking :success, id: 1, project_id: @project.identifier, resource_booking: rb_params
  end
  ...
  def test_validation_by_issue_estimated_hours
    ...
    assert Issue.find(rb_params[:issue_id]).update_attributes(estimated_hours: 200)
    should_not_update_resource_booking :success, id: 1, resource_booking: rb_params
    should_not_update_resource_booking :success, id: 1, project_id: @project.identifier, resource_booking: rb_params
  end
