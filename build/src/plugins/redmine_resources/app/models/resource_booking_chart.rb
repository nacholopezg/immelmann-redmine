...
class ResourceBookingChart < Redmine::Helpers::Gantt
  ...
  HOURS_PER_DAY = 'hours_per_day'.freeze
  TOTAL_HOURS = 'total_hours'.freeze
  ISSUE_SUBJECT = 'issue_subject'.freeze
  LINE_TITLE_TYPES = [HOURS_PER_DAY, TOTAL_HOURS, ISSUE_SUBJECT].freeze

  delegate :user_bar_label, :total_hours_bar_label, :hours_per_day_bar_label,
           :new_booking_path, :edit_path_for, :update_path_for,
           to: :view
  ...
  def initialize(project, query, options = {})
    ...
    @subjects = ''
    @lines = ''
    @editable_css_class = 'editable' if User.current.allowed_to?(:edit_booking, nil, global: true)
  end
  ...
  def render(options = {})
    ...
    @height = nil
    @bar_title_css_class = 'issue-subject-title' if @query.line_title_type == ISSUE_SUBJECT
  ...
  def render_resource_bookings_by_user(user, resource_bookings, options = {})
    ...
    increment_indent(options) do
      ...
      project = allowed_projects.detect { |project| user.member_of?(project) }
  ...
  def render_resource_bookings_by_project(resource_bookings, options = {})
    ...
    increment_indent(options) do
      if @query.show_issues
        bookings = resource_bookings.group_by(&:issue_id)
  ...
  def subject_for_nilclass(object, options)
  ...
  def line_for_issue(issue, options)
    if issue.is_a?(Issue)
      options[:resource_bookings].each do |rb|
        ...
        options[:task_bar_class] = 'task_done'
  ...
  def line_for_nilclass(object, options)
  ...
  def resource_bookings
    @resource_bookings ||= @query.resource_bookings_between(@date_from, @date_to)
  end
  ...
  def html_task_for_issue(params, coords, markers, label, object)
    output = ''
    ...
    output << view.content_tag(:div, '', class: 'split-line')
    ...
    view.content_tag(
      ...
      output.html_safe,
      style: style,
      class: "booking-bar #{@editable_css_class}",
      edit_url: edit_path_for(params[:resource_booking]),
      update_url: update_path_for(params[:resource_booking]),
      'data-split-url' => view.split_resource_booking_path(params[:resource_booking]),
    )
  end

  def html_task_for_project(params, coords, markers, label, object)
    output = ''
    css = 'task project'
    versions = params[:versions]

    if @query.show_versions
      if versions.present? && coords[:bar_start] && coords[:bar_end]
        width = coords[:bar_end] - coords[:bar_start] - 2
        style = "left:#{coords[:bar_start]}px;"
        style << "width:#{width}px;"
        content_opt = { style: style, class: "#{css} task_todo" }
        output << view.content_tag(:div, '&nbsp;'.html_safe, content_opt)
      end

      versions.each_with_index do |version, index|
        params[:version_max_width] = version_max_width(version, versions[index + 1])
        coords = coordinates(version.due_date, version.due_date, version.completed_percent, params[:zoom])
        output << html_task_for_version(params, coords, markers, version.name, version)
      end
    end
  ...
  def html_task_for_version(params, coords, markers, label, object)
    output = ''
    css = 'task version'
  ...
  def html_task_for_user(params, coords, markers, label, object)
    output = ''
    if coords[:bar_start] && coords[:bar_end]
  ...
  def html_task_bar(label, params, coords, object)
    style = "left:#{coords[:bar_start]}px;"
    style << "width:#{coords[:bar_end] - coords[:bar_start] - 2}px;"
  ...
  def scheduled_hours_for(date, user, resource_bookings)
    return if non_working_week_days.include?(date.cwday)
  ...
  def show_project_lines
    @show_project_lines ||= @query.show_versions || @query.show_issues
  end
  ...
  def build_workday_length_grouped_by_users
    ...
    if RedmineResources.people_plugin_installed?
      Person.includes(:information).where(id: user_ids).inject({}) { |h, p| h[p.id] = p.workday_length; h }
    else
  ...
  def issue_bar_label(resource_booking)
    case @query.line_title_type
    when ISSUE_SUBJECT
      resource_booking.issue.try(:subject)
    when TOTAL_HOURS
      total_hours_bar_label(resource_booking)
    else
      hours_per_day_bar_label(resource_booking)
    end
  end
end
